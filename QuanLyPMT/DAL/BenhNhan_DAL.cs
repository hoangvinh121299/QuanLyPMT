...
namespace DAL
{
    public class BenhNhan_DAL
    {
        ...
        public DataSet getHistoryExamByID(int MABENHNHAN)
        {
            ...
            string getQuerry = "select NGAYLAP, NGAYTAIKHAM, NHANVIEN.HOTEN, NHANVIEN.CHUCVU from BENHAN, BENHNHAN,NHANVIEN where BENHNHAN.MABN = BENHAN.MABN AND BENHAN.MANV=NHANVIEN.MANV AND BENHNHAN.MABN = @MABENHNHAN";
            using (SqlConnection connection = new SqlConnection(connectionString.connectionstring))
            {
                connection.Open();
